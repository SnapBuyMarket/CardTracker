...
title CardTracker - AutoMove Watcher (HEIC-enabled)
...
REM Hard override (uncomment and edit if needed):
...
echo [%date% %time%] ===== AutoMove started in %PROJECT_DIR% (HEIC) =====>> "%LOG%"

:loop
for %%F in ("%IN_DIR%\*.jpg" "%IN_DIR%\*.jpeg" "%IN_DIR%\*.png" "%IN_DIR%\*.tif" "%IN_DIR%\*.tiff" "%IN_DIR%\*.heic" "%IN_DIR%\*.HEIC") do (
  if exist "%%~fF" (
